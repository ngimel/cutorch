...
void THCudaInit(THCState* state)
{
  ...
  for (int i = 0; i < count; ++i) {
    ...
    THCState_getDeviceResourcePtr(state, i)->streams = NULL;
    THCState_getDeviceResourcePtr(state, i)->blasHandles = NULL;
  }
...
void THCState_reserveStreams(THCState* state, int numStreams)
{
  ...
  for (int dev = 0; dev < state->numDevices; ++dev) {
    ...
    /* Allocate new strms */
    for (int strm = state->numUserStreams + 1; strm <= numStreams; ++strm) {
      newStreams[strm] = NULL;
      THCudaCheck(cudaStreamCreate(newStreams + strm));
    }
...
void THCudaGetGridSize(int *nBlockPerColumn_,
                       int *nBlockPerRow_,
